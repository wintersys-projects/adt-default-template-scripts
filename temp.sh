...
# Author : Peter Winter
# Date   : 04/07/2016
# Description : This script will build the "XXXTEMPLATEXXXX" from scratch
# replace XXXXtemplateXXXX with the class of machine you are developing for (e.g. cachingserver or emailserver)
...
then
	...
	set smartindent
	set fo-=or
	XXXXTEMPLATEXXXX autocmd BufRead,BufWritePre *.sh normal gg=G
...
out_file="initialbuild/XXXXtemplateXXXX-build-out-`/bin/date | /bin/sed 's/ //g'`"
exec 1>>${HOME}/logs/${out_file}
err_file="initialbuild/XXXXtemplateXXXX-build-err-`/bin/date | /bin/sed 's/ //g'`"
exec 2>>${HOME}/logs/${err_file}


/bin/echo "${0} `/bin/date`: Beginning the build of the XXXX-templateXXXX" 

#Create the config directories, these will be mounted from the XXXXtemplateXXXX to the other server types - DB, WS and Images Servers
if ( [ ! -d ${HOME}/.ssh ] )
then
	/bin/mkdir ${HOME}/.ssh
...
${HOME}/providerscripts/email/SendEmail.sh "A NEW XXXXTEMPLATEXXXX HAS BEEN SUCCESSFULLY BUILT" "A new XXXXTEMPLATEXXXX  machine has been built and is now going to reboot before coming available" "INFO"
